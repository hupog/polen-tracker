...
public class MessagingConfig {
    ...
    Queue completedQueue() {
        return QueueBuilder.durable(COMPLETED_QUEUE).deadLetterExchange(DLX).deadLetterRoutingKey("collection.results.failed").build();
    }

    @Bean
    Queue failedQueue() {
        return QueueBuilder.durable(FAILED_QUEUE).deadLetterExchange(DLX).deadLetterRoutingKey("collection.results.failed").build();
    }
    ...
    @Bean
    Binding completed() {
        return BindingBuilder.bind(completedQueue()).to(pollenExchange()).with("collection.completed");
    }

    @Bean
    Binding failed() {
        return BindingBuilder.bind(failedQueue()).to(pollenExchange()).with("collection.failed");
    }
